...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            running = False
        # If the user presses a key, move the camera accordingly
            ...
            if event.key == pygame.K_LEFT:
                ...
                camera_y += CAMERA_SPEED
        # If the user clicks the mouse button, place or remove a block accordingly
        elif event.type == pygame.MOUSEBUTTONDOWN:
            # Get the mouse position and convert it to block coordinates
            mouse_x, mouse_y = pygame.mouse.get_pos()
            block_x = (mouse_x + camera_x) // BLOCK_SIZE
            block_y = (mouse_y + camera_y) // BLOCK_SIZE
            
            # Check if the block coordinates are valid and within bounds 
            if block_x >= 0 and block_x < COL_COUNT and block_y >= 0 and block_y < ROW_COUNT:
                # If the user clicks the left mouse button, place a red block 
                if event.button == pygame.BUTTON_LEFT:
                    blocks[block_y][block_x] = RED
                
                # If the user clicks the right mouse button, remove the block 
                elif event.button == pygame.BUTTON
